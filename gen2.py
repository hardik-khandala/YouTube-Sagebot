...
def speak_text(text):
    # Convert text to speech using Google TTS with enhanced female voice
    try:
        tts = gTTS(text, lang='en', slow=False, tld='com')
        audio_buffer = io.BytesIO()
        tts.write_to_fp(audio_buffer)
        audio_buffer.seek(0)  # Reset buffer position to the beginning
        st.audio(audio_buffer, format="audio/mp3")  # Play audio in Streamlit
    except Exception as e:
    ...
    with col1:
        if st.button("Process"):
            if link and transcript:
                ...
                extended_prompt = f"""Give answers only based on the following question and context, extend the given answer: {qa_answer}\n\nQuestion: {question}\nContext: {context} and if the question is not related to our Context then tell me that, "please enter a valid question related to your video" """
