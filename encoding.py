    ...
    with col1:
        ...
        if st.button("Start Recording"):
            if not st.session_state.recording:
                start_recording()
                st.text_area("Question:", value=st.session_state.voice_input, height=300)
            else:
        ...
        if st.button("Process"):
            if link and transcript:
                ...
                if not qa_answer or qa_answer.lower() == "sorry, no valid answer found.":
                    st.warning("Please enter a valid question related to the video.")
                else:
                    extended_prompt = f"""Give answers only based on the following question and context, extend the given answer: {qa_answer}\n\nQuestion: {corrected_question}\nContext: {context}   and after this response tell me that "let me know if you need any kind of help to assist you with this video" and if the question is not related to our Context then tell me that, "please enter a valid question related to your video"  """
